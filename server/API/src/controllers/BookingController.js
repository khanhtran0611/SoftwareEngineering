...
class BookingController {
   ...
   ViewBooking(req,res){
    ...
    CheckBookingStatus(req, res) {
        const room_id = req.params.room_id;
        pool.query(queries.checkBookingStatus, [room_id], (err, result) => {
            if(err){
                return res.status(500).json({
                    status : 0,
                    message: err.message
            ...
            if(result.rows.length > 0){
                return res.status(200).json({
                    status : 1,
                    message: 'Đã có booking cho phòng này'
                });
            }
            else{
                return res.status(200).json({
                    status : 0,
                    message: 'Không có booking cho phòng này'
                });
            }
        })
    }
