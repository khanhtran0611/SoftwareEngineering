...
import { getHotelRooms } from "./rooms"
...
export async function checkBookingStatus(roomId: number) {
  try {
    const response = await api.get(`/api/check-booking-status/${roomId}`);
...
export async function checkBookingStatusbyHotel(hotel : Hotel){
  try {
    let rooms = await getHotelRooms(hotel.hotel_id) 
