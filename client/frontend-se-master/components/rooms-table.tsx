...
export default function RoomsTable({ hotelId, searchQuery }: RoomsTableProps) {
  ...
  const handleAddRoom = (newRoom: Room | Omit<Room, "room_id">) => {
    if ('room_id' in newRoom) {
  ...
  const handleDeleteRoom = async () => {
    ...
    try {
      const response = await checkBookingStatus(selectedRoom.room_id)
      if(response){
        toast({
          title: "Error",
          description: "The room is booked",
          variant: "destructive"
        })
        return
      }
