...
export default function HotelOwnerHeader() {
  ...
  const isHotelsPage = pathname === "/hotel/dashboard" || pathname === "/hotel/dashboard/"
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Pass search query to parent component or use context
    window.dispatchEvent(new CustomEvent("hotelsearch", { detail: e.target.value }))
  }

  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    // Handle search submission
    window.dispatchEvent(new CustomEvent("hotelsearch", { detail: searchQuery }))
  }
      ...
      <div className="container mx-auto flex h-16 items-center justify-between px-4">
        <Link href="/hotel/dashboard" className="flex items-center gap-2">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs text-muted-foreground">Hotel Owner Dashboard</span>
          </div>
        </Link>
