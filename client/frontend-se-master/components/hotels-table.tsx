...
import { checkBookingStatusbyHotel } from "@/lib/bookings"
...
export default function HotelsTable({ searchQuery }: HotelsTableProps) {
  ...
  const handleDeleteHotel = async () => {
    ...
    setIsDeleting(true)
    try {
      const response = await checkBookingStatusbyHotel(selectedHotel)
      if(response){
        toast({
          title: "Error",
          description: "A hotel's room is booked",
          variant: "destructive"
        })
        return
      }
                ...
                <TableRow key={hotel.hotel_id}>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1">
                      ...
                      <span>{hotel.rating ? hotel.rating : "No review yet"}</span>
                    </div>
                  </TableCell>
